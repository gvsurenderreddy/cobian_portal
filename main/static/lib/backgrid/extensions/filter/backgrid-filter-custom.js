...
(function (factory) {
  ...
  */
  var ServerSideFilter = Backgrid.Extension.ServerSideFilter = Backbone.View.extend({
	...
	template: _.template('<span class="input-group-addon"><span class="glyphicon glyphicon-search"></span></span><input type="search" class="form-control" <% if (placeholder) { %> placeholder="<%- placeholder %>" <% } %> name="<%- name %>" /><span data-backgrid-action="clear" class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>', null, {variable: null}),

    /** @property */
    events: {
      "keyup input[type=search]": "showClearButtonMaybe",
      "click span[data-backgrid-action=clear]": "clear",
      "submit": "search"
    },
    ...
    */
    ...
    clearButton: function () {
      return this.$el.find("span[data-backgrid-action=clear]");
    },
